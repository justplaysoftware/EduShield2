...
//
// JustPlay Software, June 2018
// v1.0.5 June 2018 - fixed setRGBColor( String )
// added some functions to support PulseRate
#include "Arduino.h"
...
void EduShield2_7segment::pulseRate_Observe() {
  int test = 0;

  pr_val = analogRead(pr_pulsePin);
  if ((pr_val > pr_threshold) && (pr_val < pr_maxPulse)) {
    if (pr_reading > (_JP_PR_MAX_READINGS - 1)) {
      pr_reading = 0;
    }
    if ((pr_val > pr_maxPulse) && (pr_val < 800)) pr_maxPulse = pr_val;
    if ((pr_val < pr_threshold) && (pr_val > 200)) pr_threshold = pr_val;
    // we have a pr_valid pr_reading
    test = map(pr_val, pr_threshold, pr_maxPulse, 0, 255);
    pr_vals[pr_reading] = test;
    pr_reading++;
    pr_ave = calcAve();
    if ((test > (pr_ave * 1.4)) && (pr_sawBeat == false)) {
      pr_sawBeat = true;
      pr_beats++;
      if (pr_displayLed) analogWrite(pr_fadePin, test);
	  if (pr_beep) tone( 4, 460, 80);
    }
    else if ((test < pr_ave) && (pr_sawBeat == true)) {
      if (pr_displayLed) analogWrite(pr_fadePin, test);
      pr_sawBeat = false;
    }
    else if (test <= pr_ave) {
      pr_sawBeat = false;
      if (pr_displayLed) analogWrite(pr_fadePin, 0);
    }
  }
  
    if (pr_displayVals) printVals(test);
	
  delay(pr_delayTime);
}

void EduShield2_7segment::printVals(int pr_test) {
	pr_last = (millis() - pr_start) / 1000;

	pr_bpm = (int)(((float)pr_beats / (float)pr_last) * 60.0);
	Serial.print( pr_bpm );
	Serial.print( ", ");
	Serial.print( pr_test );
	Serial.print( ", ");
	Serial.println( pr_ave );
	
}

int EduShield2_7segment::pulseRate_getBPM() {
	return pr_bpm;
}

int EduShield2_7segment::calcAve() {
    long tot = 0;
    int ret = 0;
  
    for (int y = 0; y < _JP_PR_MAX_READINGS; y++) {
      tot = tot + pr_vals[y];
    }

    ret = (int)(tot / _JP_PR_MAX_READINGS);
    return ret;
}


void EduShield2_7segment::pulseRate_Startup(int pulsePin, int delayTime, bool displayLed, bool displayVals, bool beep) {
	pr_delayTime = delayTime;
	pr_pulsePin = pulsePin;
	pr_beep = beep;
	pr_displayVals = displayVals;
	pr_displayLed = displayLed;
  int test = 0;
    pinMode(pulsePin, INPUT);
  pinMode(pr_fadePin, OUTPUT);
  ...
  for( int z=0; z<300; z++) {
    ...
    if ((pr_val > pr_threshold) && (pr_val < pr_maxPulse)) {
      if ((pr_val > pr_maxPulse) && (pr_val < 800)) pr_maxPulse = pr_val;
      if ((pr_val < pr_threshold) && (pr_val > 200)) pr_threshold = pr_val;
      // we hpr_ave a pr_valid pr_reading
      test = map(pr_val, pr_threshold, pr_maxPulse, 0, 255);
      pr_vals[z] = test;
      z++;
    } else {
